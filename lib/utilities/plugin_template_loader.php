 ...
 *
 * @package   Plugin_Template_Loader
 * @version   1.0.0
 */
if ( ! class_exists( 'Plugin_Template_Loader' ) ) {
	/**
	 * Template loader.
	 *
	...
	class Plugin_Template_Loader {
		 ...
		 * @return string
		 */
		public function get_template_part( $slug, $name = null,  $format = null, $load = true ) {
			// Execute code for this part.
			do_action( 'get_template_part_' . $slug, $slug, $name );
			do_action( $this->filter_prefix . '_get_template_part_' . $slug, $slug, $name );
			// Get files names of templates, for given slug and name.
			$templates = $this->get_template_file_names( $slug, $name, $format );
		 ...
		 * @return array
		 */
		protected function get_template_file_names( $slug, $name, $format ) {
			$templates = array();
			if ( isset( $name ) ) {
				$templates[] = $slug . '-' . $name . '.php';
			}
			if ( isset( $format ) ) {
				$templates[] = $slug . '-' . $name . '-' . $format . '.php';
				$templates[] = $slug . '-' . $format . '.php';
			}
			$templates[] = $slug . '.php';
			/**
